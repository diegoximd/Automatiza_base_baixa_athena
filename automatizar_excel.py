...
# === Configurações ===
arquivo_excel_origem = r"C:\Users\Suporte\Desktop\Mercado\Athena Saude\PAGAMENTO HUMANA NE - 01A15092025.xlsx"
arquivo_excel_saida = r"C:\Users\Suporte\Desktop\Mercado\Athena Saude\HUMANA_NE_ARQUIVO_BAIXA_800_ATHENA.xlsx"
arquivo_sql_saida = r"C:\Users\Suporte\Desktop\Mercado\Athena Saude\HUMANA_NE_ARQUIVO_BAIXA_800_ATHENAs.sql"
banco_id = 2002
...
# Extrai a data do nome do arquivo (procura 8 dígitos seguidos)
nome_arquivo = os.path.basename(arquivo_excel_origem)
match = re.search(r"(\d{8})", nome_arquivo)
if not match:
    raise ValueError("❌ Não foi encontrada uma data (ddmmaaaa) no nome do arquivo!")

data_str = match.group(1)  # Ex: '15092025'
...
# Preenche cabeçalho com data e remessa
ws.cell(row=2, column=1, value=data_formatada)  # Dt. Remessa
ws.cell(row=2, column=2, value=remessa)        # Número da Remessa
ws.cell(row=2, column=3, value=banco_id)       # Código da Empresa
